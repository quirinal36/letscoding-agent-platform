...
export const policyVersionSchema = {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: "https://schemas.letscoding.kr/agent-platform/shared/policy-version.schema.json",
    title: "PolicyVersion",
    description: "발행일과 그날의 발행 순번으로 이루어진 불변 버전. 예: 2026-08-12.3. 이 pattern은 달력에 존재하지 않는 날짜를 거르지 못하므로 파서가 추가로 검사한다.",
    type: "string",
    pattern: "^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])\\.([1-9][0-9]*)$",
};
export const policyCheckCodeSchema = {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: "https://schemas.letscoding.kr/agent-platform/shared/error-code.schema.json",
    title: "PolicyCheckCode",
    description: "사용자 작품 판정에 사용하는 안정적 오류 코드. 한번 발행한 코드는 재사용·개명·의미 변경을 하지 않는다.",
    type: "string",
...
export const policyDocumentSchema = {
    ...
    description: "발행된 Lounge Deploy 정책 스냅샷 하나의 계약. 발행 후에는 수정하지 않는다.",
    type: "object",
    ...
    properties: {
        ...
        effectiveAt: {
            description: "정책 시행 시각(UTC). 버전 문자열의 날짜는 발행일 표기이며 시행 시각이 아니다.",
            type: "string",
            format: "date-time",
        },
        changeReason: {
        ...
        zip: {
            ...
            properties: {
                ...
                allowSingleWrapperDirectory: {
                    description: "단일 wrapper 폴더를 제거한 뒤의 루트를 허용하는지 여부.",
                    type: "boolean",
                },
                allowZip64: {
                    type: "boolean",
        ...
        files: {
            ...
            properties: {
                ...
                blockedFilenames: {
                    ...
                    items: {
                        ...
                        properties: {
                            ...
                            scope: {
                                description: "basename만 비교할지 경로의 모든 세그먼트를 비교할지 지정한다. 생략 시 basename이다.",
                ...
                blockedPathRules: {
                    description: "차단 경로 규칙. 정책은 규칙 종류만 선언하고 해석은 검증기 코드가 담당한다.",
                    type: "array",
                    minItems: 1,
                    uniqueItems: true,
                    items: {
        ...
        runtimeEnv: {
            ...
            description: "런타임 환경변수 처리 규칙. ZIP 차단 자체는 files.blockedFilenames가 담당한다.",
            type: "object",
            additionalProperties: false,
            required: [
                "attachSeparately",
            ...
            items: {
                ...
                properties: {
                    ...
                    detect: {
                        ...
                        description: "감지 입력 데이터. 감지 알고리즘 자체는 이 계약의 범위가 아니다.",
                        type: "object",
                        additionalProperties: false,
                    ...
                    artifactKinds: {
                        description: "이 프레임워크에서 1차 에이전트가 만들 수 있는 업로드 산출물.",
                        type: "array",
                        minItems: 1,
                        uniqueItems: true,
                        items: {
                    ...
                    checks: {
                        description: "이 프레임워크에서 추가로 적용하는 checks[].code 참조.",
                        type: "array",
                        uniqueItems: true,
                        items: {
                            $ref: "https://schemas.letscoding.kr/agent-platform/shared/error-code.schema.json",
        ...
        checks: {
            description: "오류 코드 카탈로그. 다른 절의 code 값은 모두 여기에 존재해야 한다.",
            type: "array",
            minItems: 1,
            items: {
                title: "PolicyCheck",
                ...
                properties: {
                    ...
                    waivable: {
                        description: "사용자가 사유를 확인한 뒤 해제할 수 있는지 여부. severity가 error면 false여야 한다.",
                        type: "boolean",
                    },
                    appliesTo: {
                    ...
                    titleKey: {
                        description: "표시 문구 자원의 키. 사람이 읽는 문장은 정책에 넣지 않는다.",
                        type: "string",
...
export const currentPointerSchema = {
    $schema: "https://json-schema.org/draft/2020-12/schema",
    $id: "https://schemas.letscoding.kr/agent-platform/current-pointer.schema.json",
    title: "CurrentPointer",
    description: "활성 정책 스냅샷을 가리키는 포인터. 정책 규칙 값을 복제하지 않고, 스냅샷 경로는 history/<version>.json 규약으로 파생한다.",
    type: "object",
    additionalProperties: false,
...
export const allPolicyContractSchemas = [
    policyIdSchema,
    policyVersionSchema,
