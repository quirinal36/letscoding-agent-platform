 ...
 */
export function createZipFixture(entries, mutation = {}) {
    const localParts = [];
    const centralParts = [];
    let localOffset = 0;
    entries.forEach((entry, index) => {
        ...
        const localNameText = index === 0 && mutation.localNameOverride !== undefined
            ? mutation.localNameOverride
            : entry.name;
        const localName = Buffer.from(localNameText, useUtf8 ? "utf8" : "ascii");
        const entryCrc = crc32(contents);
        const declaredUncompressed = index === 0 && mutation.declaredUncompressedBytes !== undefined
    ...
    eocd.writeUInt16LE(mutation.forceZip64Marker === true ? 0xffff : entries.length, 8);
    eocd.writeUInt16LE(mutation.forceZip64Marker === true ? 0xffff : entries.length, 10);
    eocd.writeUInt32LE(central.length, 12);
    eocd.writeUInt32LE(centralOffset + (mutation.centralDirectoryOffsetDelta ?? 0), 16);
...
export async function generateArtifactSecurityFixtures(outputDirectory) {
    ...
    const fixtures = {
        ...
        "invalid-central-directory.zip": createZipFixture([{ name: "index.html", contents: "<!doctype html>" }], { centralDirectoryOffsetDelta: 1 }),
        "invalid-local-name.zip": createZipFixture([{ name: "index.html", contents: "<!doctype html>" }], { localNameOverride: "other.html" }),
        "invalid-multidisk.zip": createZipFixture([{ name: "index.html", contents: "<!doctype html>" }], { diskNumber: 1 }),
        "invalid-zip64.zip": createZipFixture([{ name: "index.html", contents: "<!doctype html>" }], { forceZip64Marker: true }),
        "invalid-truncated.zip": createZipFixture([{ name: "index.html", contents: "<!doctype html>" }], { truncateBytes: 8 }),
    };
    const written = [];
...
//# sourceMappingURL=fixture-generator.js.map