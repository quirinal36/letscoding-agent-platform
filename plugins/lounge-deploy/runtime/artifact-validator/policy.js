import { checkMessageKo, } from "@letscoding/policy-contract";
const INTERNAL_RULES = {
    "artifact-type-invalid": internalRule("ARTIFACT_TYPE_INVALID", "Artifact 종류가 올바르지 않습니다."),
    "compressed-size-required": internalRule("ARTIFACT_COMPRESSED_SIZE_REQUIRED", "ZIP 압축 크기가 필요합니다."),
    "compressed-size-invalid": internalRule("ARTIFACT_COMPRESSED_SIZE_INVALID", "ZIP 압축 크기가 올바르지 않습니다."),
    "file-size-invalid": internalRule("ARTIFACT_FILE_SIZE_INVALID", "파일 크기 metadata가 올바르지 않습니다."),
    "total-size-overflow": internalRule("ARTIFACT_TOTAL_SIZE_OVERFLOW", "전체 파일 크기를 안전하게 표현할 수 없습니다."),
    "sha256-invalid": internalRule("ARTIFACT_SHA256_INVALID", "파일 SHA-256 metadata가 올바르지 않습니다."),
    "path-invalid": internalRule("ARTIFACT_PATH_INVALID", "파일 경로 metadata가 올바르지 않습니다."),
    "path-duplicate": internalRule("ARTIFACT_PATH_DUPLICATE", "같은 파일 경로가 중복되었습니다."),
    "path-case-collision": internalRule("ARTIFACT_PATH_CASE_COLLISION", "대소문자만 다른 파일 경로가 충돌합니다."),
    "warning-waiver-invalid": internalRule("ARTIFACT_WARNING_WAIVER_INVALID", "경고 해제 code 또는 사유가 올바르지 않습니다."),
};
/** Convert the versioned central policy into the deterministic validator API. */
export function artifactValidationPolicyFromDocument(document) {
    const checks = new Map(document.checks.map((check) => [check.code, check]));
    const ruleFor = (code) => policyRule(requireCheck(checks, code));
    ...
    const pathRules = mutablePathRules;
    const blockedFilenameRules = document.files.blockedFilenames.map((entry) => ({
        match: entry.match,
    ...
    const rules = {
        ...
        "uncompressed-size-exceeded": ruleFor(document.zip.codes.uncompressedTooLarge),
    ...
    if (document.zip.requireRootIndexHtml) {
        rules["root-file-missing"] = ruleFor(document.zip.codes.missingRootIndexHtml);
    }
    return {
        id: document.id,
        ...
        paths: {
            ...
            forbidDotSegments: pathRules.parentTraversal !== undefined ||
            ...
            forbiddenUrlCharacters: pathRules.urlReinterpret === undefined ? "" : "%?#;",
            rules: pathRules,
        },
        structure: {
            rootFile: "index.html",
...
function assignRule(rules, ruleId, rule) {
    if (rule !== undefined)
        rules[ruleId] = rule;
}
function requireCheck(checks, code) {
...
function policyRule(check) {
    return {
        code: check.code,
        severity: check.severity,
        waivable: check.waivable,
        message: checkMessageKo(check.titleKey) ??
            "정책 검사에 대응하는 표시 문구가 없습니다.",
    };
}
function internalRule(code, message) {
