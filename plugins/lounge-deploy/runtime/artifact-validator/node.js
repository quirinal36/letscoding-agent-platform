...
export async function inspectArtifact(options) {
    try {
...
async function inspectZip(options) {
    ...
    const eventTargetHandle = handle;
    setMaxListeners(Math.max(10, options.policy.limits.maxFiles + 1), eventTargetHandle);
    try {
        const directory = await readZipDirectory(handle, compressedBytes, options.policy.limits.maxEntries ?? options.policy.limits.maxFiles);
        let actualTotal = 0;
        let declaredTotal = 0;
        const files = [];
        for (const [entryIndex, entry] of directory.entries.entries()) {
            ...
            const remaining = options.policy.limits.maxUncompressedBytes - actualTotal;
            const hashed = await hashZipEntry(handle, directory.fileSize, entry, entryIndex, remaining);
...
async function inspectDirectory(options) {
    ...
    const candidates = await collectDirectoryFiles(absoluteRoot, canonicalRoot, options.policy.limits.maxFiles);
    const files = [];
    let total = 0;
    ...
    const manifest = { kind: "directory", files };
    return validatedResult(options, manifest, null, total);
}
async function collectDirectoryFiles(root, canonicalRoot, maxFiles) {
    const files = [];
...
async function hashDirectoryFile(path, snapshot) {
...
async function hashZipEntry(handle, fileSize, entry, entryIndex, maxOutputBytes) {
...
function validatedResult(options, manifest, sourceSha256, uncompressedBytes) {
    ...
    return {
        ...
        metadata: {
            kind: options.kind,
            sourceSha256,
            compressedBytes: manifest.kind === "zip" ? (manifest.compressedBytes ?? null) : null,
            uncompressedBytes,
...
function inspectionFailure(options, findings, partial) {
    return {
        pass: false,
        ...
        inspectionErrors: findings.map((finding) => attachPolicyRule(finding, options.policy)),
        validation: null,
    };
}
function attachPolicyRule(finding, policy) {
    const policyRule = inspectionPolicyRule(finding.code, policy);
    return policyRule === undefined ? finding : { ...finding, policyRule };
}
function inspectionPolicyRule(code, policy) {
...
function inspectionError(code, entryIndex, metadata) {
    const finding = {
        code,
...
const INSPECTION_MESSAGES = {
    ...
    ARTIFACT_DIRECTORY_SPECIAL_FILE: "출력 폴더에 일반 파일이 아닌 항목이 있습니다.",
    ...
    ZIP_FILENAME_ENCODING_UNSUPPORTED: "ZIP 파일명 인코딩을 안전하게 해석할 수 없습니다.",
    ...
    ZIP_ENTRY_SIZE_MISMATCH: "ZIP entry 크기 metadata와 실제 결과가 일치하지 않습니다.",
...
function validateEntryFlagsAndMethod(flags, method, entryIndex) {
...
function decodeEntryName(bytes, flags, entryIndex) {
